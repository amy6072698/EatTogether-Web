    ...
    <div class="modal-box">
      ...
      <div class="detail-body">
        ...
        <div class="detail-row-1">
          <h2 class="font-headline detail-name italic" style="color:#e3c76b">{{ dish.productName }}</h2>
          <div class="detail-tags">
            <span v-for="tag in tags" :key="tag.key" :class="['badge', tag.cls]">{{ tag.label }}</span>
          </div>
        </div>
        <!-- 第二排：描述 -->
        <p class="font-body detail-desc italic leading-relaxed" style="color:rgba(249,221,211,0.7)">{{ dish.description }}</p>
        <!-- 第三排：價格（左）+ 數量控制（右） -->
        <div class="detail-row-3">
          <span class="font-label detail-price" style="color:#d5b478">NT$ {{ dish.unitPrice?.toLocaleString() }}</span>
          <div class="detail-qty">
            <button class="qty-btn" @click="localQty = Math.max(1, localQty - 1)">−</button>
            <span class="font-label detail-qty-num" style="color:#f9ddd3">{{ localQty }}</span>
