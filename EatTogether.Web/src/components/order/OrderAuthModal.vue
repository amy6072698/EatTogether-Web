        ...
        <div class="auth-overlay">
            <div class="auth-box">
              ...
              登入會員
                        ...
                        <!-- 先帶入Id=61的會員 -->
                        <button
                            class="choice-btn choice-login font-label"
                            @click="$emit('logged-in', { name: '冷明輝', memberId: 61 })"
                        >
                            <svg
                ...
                <template v-else>
                    <button
                        class="back-btn font-label"
                        @click="(step = 'choice')((loginError = ''))"
                    >
                        ← 返回
                    </button>
...
async function doLogin() {
    ...
    try {
        ...
        if (!res.ok) {
            ...
            loginError.value =
                body.message ||
                (res.status === 401 ? '帳號或密碼錯誤' : `登入失敗（${res.status}）`)
            return
        }
